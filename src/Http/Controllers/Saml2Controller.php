...
namespace Saml\Bearer\Http\Controllers;

use Saml\Bearer\Events\Saml2LoginEvent;
...
class Saml2Controller extends Controller
{
    ...
    public function acs(Saml2Auth $saml2Auth, $idpName)
    {
        ...
        $model = $userModel::where('saml_id', $user->getAttribute('id')[0])->first();

        event(new Saml2LoginEvent($idpName, $user, $saml2Auth, $model));

        // Auth::guard(config('saml_bearer.saml_guard'))->setUser();
